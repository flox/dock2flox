...
# --- Heredoc per-statement processing helpers ---
# Split heredoc RUN bodies into individual statements before the safety gate.
# NOTE: Also in parser_dockerfile.heredoc-per-statement.sh for standalone testing.
...
_parse_run() {
    ...
    local encoded_run_body run_body run_body_is_heredoc=0
    encoded_run_body=$(echo "$line" | sed -E 's/^RUN\s+//i')
    if _d2f_run_body_encoded_from_heredoc "$encoded_run_body"; then
    ...
    if [[ "$run_body_is_heredoc" -eq 1 && -z "${D2F_RUN_HEREDOC_STATEMENT_PARSE:-}" ]]; then
        _parse_run_heredoc_per_statement "$run_body" "$ir_file" "$line_num"
        return 0
    fi
...
_run_body_can_use_conservative_fast_path() {
    ...
    # Heredoc statements with context may contain newlines; force through interpreter.
    [[ "$body" == *$'\n'* ]] && return 1
